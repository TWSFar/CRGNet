...
class MakeDataset(object):
    ...
    def write_chip_and_anno(self, image, img_id, chip_list):
        ...
        for i, chip in enumerate(chip_list):
            ...
            remainder[chip[1]:chip[3], chip[0]:chip[2], :] = 0
        ...
        if args.supplement:
            if image.shape[1] > image.shape[0]:
                img_name = '{}_{}.jpg'.format(img_id, i+1)
                cv2.imwrite(osp.join(self.chip_dir, img_name), remainder[:, :image.shape[1]//2+50, :])
                chip_loc[img_name] = [0, 0, image.shape[1]//2+50, image.shape[0]]
                img_name = '{}_{}.jpg'.format(img_id, i+2)
                cv2.imwrite(osp.join(self.chip_dir, img_name), remainder[:, image.shape[1]//2-50:, :])
                chip_loc[img_name] = [image.shape[1]//2-50, 0, image.shape[1], image.shape[0]]
            else:
                img_name = '{}_{}.jpg'.format(img_id, i+1)
                cv2.imwrite(osp.join(self.chip_dir, img_name), remainder[:image.shape[0]//2+50, :, :])
                chip_loc[img_name] = [0, 0, image.shape[1], image.shape[0]//2+50]
                img_name = '{}_{}.jpg'.format(img_id, i+2)
                cv2.imwrite(osp.join(self.chip_dir, img_name), remainder[image.shape[0]//2-50:, :, :])
                chip_loc[img_name] = [0, image.shape[0]//-50, image.shape[1], image.shape[0]]
