...
class Statistics(object):
    ...
    def density_distribution(self, maxn=20):
        print("statistics train + val datasets density distribution")
        print("denisty value limited in [0: {})".format(maxn))
        density_value = np.zeros(maxn).astype(np.int64)
        for filename in tqdm(os.listdir(self.segmentation_dir)):
            filepath = osp.join(self.segmentation_dir, filename) 
            with h5py.File(filepath, 'r') as hf:
                mask = np.array(hf['label']).astype(np.int64)
                density_value += np.bincount(mask.ravel(), minlength=maxn)[:maxn]

        with open(osp.join(result_dir, 'density_distribution.txt'), 'w') as f:
            for i, number in enumerate(density_value):
                f.writelines(str(i) + ': ' + str(number) + '\n')
        ...
        plt.ylabel('number')
        plt.bar(range(len(density_value)), density_value)
